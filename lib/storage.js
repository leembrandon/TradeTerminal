...
export async function migrateLocalData(user) {
  if (!user) return { playbooks: 0, sessions: 0 };
    ...
    const results = await Promise.allSettled(
      localSessions.map(s =>
        fetch("/api/journal", {
          ...
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify(sessionToRow(s)),
        })
      )
    );
    const ok = results.filter(r => r.status === "fulfilled" && r.value.ok).length;
  ...
  return { playbooks: playbookCount, sessions: sessionCount };
}

export function hasLocalData() {
  if (typeof window === "undefined") return { hasData: false, playbooks: 0, sessions: 0 };
  try {
    ...
    return { hasData: pbCount > 0 || jCount > 0, playbooks: pbCount, sessions: jCount };
  } catch {
    return { hasData: false, playbooks: 0, sessions: 0 };
  }
}
