...
export async function migrateLocalData(user) {
  ...
  // Migrate learning path progress — MERGE with existing cloud progress
  // so we don't overwrite lessons the user already completed before signing out.
  const localLearn = loadLearnLocal();
  const learnCount = Object.values(localLearn).filter(Boolean).length;
  if (learnCount > 0) {
    try {
      // Fetch existing cloud progress first
      let cloudLearn = {};
      try {
        const getRes = await fetch("/api/learn", { credentials: "include" });
        if (getRes.ok) {
      ...
      // Merge: union of completed lessons. Once completed, stays completed.
      const merged = { ...cloudLearn, ...localLearn };
